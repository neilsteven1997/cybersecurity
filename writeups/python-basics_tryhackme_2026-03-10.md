...
structures like if statements and consist of equivalence with ==, less than with <, less than or equal to with <=, greater than with >, and greater than or equal to with >=. Boolean operators link multiple conditions: AND demands both true, OR accepts any one true,>and NOT reverses the operand’s truth value. 
