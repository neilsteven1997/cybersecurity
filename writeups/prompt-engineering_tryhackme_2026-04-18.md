...
| Few-shot prompting example for authentication event classification | Classify these authentication events: - "User admin logged in from <IP_Address>" → NORMAL - "Failed login attempt for root from <IP_Address>" → SUSPICIOUS - "5 failed logins for user bob in 10 seconds" → ATTACK Now classify: "User guest logged in from <IP_Address> at 3:47 AM" |
